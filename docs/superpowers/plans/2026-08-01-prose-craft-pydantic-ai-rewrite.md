...
def test_voice_path_validates_name(tmp_voices_root):
    ...
    p = voice_path("dnova", root=tmp_voices_root)
    assert p == tmp_voices_root / "dnova" / "voice.md"
...
- Create: `tests/fixtures/voices/dnova/voice.md`
...
```markdown
# tests/fixtures/voices/dnova/voice.md
---
voice: dnova
version: 1
created: 2026-08-01
...
def test_voice_stylist_draft_mode(tmp_path: Path):
    ...
    deps = StylistDeps(file_path=draft, voice_name="dnova", mode="draft")
    payload = {
        "text": "Drafted text in the dnova voice.",
        "change_log": "rules_honored: diction.banned",
        "voice_check_report": None,
    }
    with agent.override(model=_function_model_returning_json(payload)):
        result = agent.run_sync("Draft.", deps=deps)
    assert "dnova" in result.output.text
...
def test_voice_check_json(monkeypatch, tmp_path):
    monkeypatch.setenv("PROSE_CRAFT_VOICES_ROOT", str(tmp_path))
    _write_voice(tmp_path, "dnova")
    draft = tmp_path / "p.md"
    draft.write_text("We will utilize this.", encoding="utf-8")
    result = runner.invoke(app, ["voice", "check", str(draft), "--voice", "dnova", "--json"])
    assert result.exit_code == 0
    import json
...
def test_voice_draft_writes_file(monkeypatch, tmp_path):
    monkeypatch.setenv("PROSE_CRAFT_VOICES_ROOT", str(tmp_path))
    (tmp_path / "dnova").mkdir()
    (tmp_path / "dnova" / "voice.md").write_text(
        "---\nvoice: dnova\nversion: 1\ncreated: 2026-08-01\nupdated: 2026-08-01\n"
        "register: {}\ndiction: {}\nrhythm: {}\nsyntax: {}\nlexicon: {}\nstructure: {}\n",
        encoding="utf-8",
    )
...
prose analyze chapter.md --voice dnova --metrics-only

# Edit prose in a voice
prose edit chapter.md --voice dnova
