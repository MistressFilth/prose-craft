...
def build_voice_checker(
    ...
    *,
    ...
    ``voices_root`` closes the read-side ``load_voice`` tool onto a
    configured root so ``--voices-root`` CLI overrides are honored at
    agent-tool time. ``None`` keeps the env-default behavior.
    """
    ...
    return make_sub_agent(
        ...
        tools=[read_file, tools["load_voice"]],
    )
