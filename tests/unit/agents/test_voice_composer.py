...
def _seed_voice_md(voice_dir: Path, body_text: str = "", name: str = "dnova") -> None:
...
def test_voice_composer_apply_voice_delta_round_trip(tmp_path: Path):
    ...
    current_json = load_tool.function(ctx, "dnova")
    current = json.loads(current_json)
    assert current["voice"] == "dnova"
    ...
    reloaded = _io_read_voice("dnova", root=voices_root)
...
def test_write_voice_with_explicit_prose_body_overwrites(tmp_path: Path):
    ...
    write_tool = toolset.tools["write_voice"]
    write_tool.function(ctx, "dnova", current_json, prose_body="\nnew body\n")
...
def test_apply_voice_delta_rejects_voice_field(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="delta.field='voice'"):
        apply_tool.function(ctx, "dnova", current_json, delta_json)
...
def test_voice_composer_uses_configured_voices_root(tmp_path: Path, monkeypatch):
    ...
    load_tool = toolset.tools["load_voice"]
    profile_json = load_tool.function(ctx, "dnova")
    assert "dnova" in profile_json
    ...
    body = read_tool.function(ctx, "dnova")
    write_tool = toolset.tools["write_voice"]
    write_tool.function(ctx, "dnova", profile_json, prose_body=body)

    # Verify the file lives at the configured root, not the env default.
    assert (custom_root / "dnova" / "voice.md").exists()
...
def test_voice_composer_end_to_end_pipeline(tmp_path: Path):
    ...
    ctx = _make_ctx(deps)
    profile_json = agent_toolset.tools["load_voice"].function(ctx, "dnova")
    delta_json = result.output[0].model_dump_json()
    updated_json = agent_toolset.tools["apply_voice_delta"].function(
        ctx, "dnova", profile_json, delta_json
    )
    agent_toolset.tools["write_voice"].function(ctx, "dnova", updated_json)

    from prose_craft.voices.io import read_voice as _io_read_voice
    from prose_craft.voices.io import read_voice_raw

    reloaded_profile = _io_read_voice("dnova", root=voices_root)
    _reloaded_profile, reloaded_body = read_voice_raw("dnova", root=voices_root)
