...
`<name>` becomes the voice's directory key under `${CLAUDE_PLUGIN_DATA}/voices/<name>/` and is stored in the profile's `voice:` field. Use lowercase letters, digits, and hyphens. Examples: `dnova`, `house-rfc`, `newsletter`.
...
mkdir -p /Users/dnova/.claude/plugins/data/prose/voices/.backups/${TS}-compose-<name>
cp -r /Users/dnova/.claude/plugins/data/prose/voices/<name> \
  /Users/dnova/.claude/plugins/data/prose/voices/.backups/${TS}-compose-<name>/
```
